...
Tavily で集めた根拠を元に、Zenn 記事を **「概要提案 → 構成案(plan)→ 本文(publish)」** の 3 段階で安定して書き上げるための Claude Code 用スキルです。AI が既存知識だけで断定することを禁じ、必ず `tav-cli` スキルで最新情報を確認させた上で、根拠 URL 付きの記事に組み立てます。

## 前提条件

- `tav-cli` スキルが存在し、Tavily API キーがセットされていること
  - 詳細は [claude-plugins/my-tools/skills/tav-cli/README.md](../../my-tools/skills/tav-cli/README.md) を参照
- リポジトリ直下に `zenn/` ディレクトリ、その配下に `plan/` と `publish/` を作成可能なこと(初回実行時に作られる想定)
- 調査結果の置き場として `<TAVILY_OUTPUT_DIR>/<topic>/`(既定 `temp/web/<topic>/`)が利用可能なこと。本スキルは **1 記事 = 1 トピック** で、すべての調査を同じ `--topic <article_topic>` に蓄積する(フォルダ内の構成・読み方は `tav-cli` スキルに従う)
...
- Web 調査は曖昧なブラウズ指示や AI 組み込み検索ではなく、`tav-cli` スキルを **明示的に使い分けさせる**
...
- **調査スキルとの分業**: Tavily の呼び方・コマンドの使い方・出力の読み方は `tav-cli` 側に任せ、本スキルは「いつどのコマンドを呼ぶか」と「成果物の形」だけ規定
...
1. `tav-cli` で軽い事前調査(主要な公式 URL を把握。以降すべて同じ `--topic <article_topic>`)
...
```text
claude-plugins/others/skills/zenn/
├── README.md   ← このファイル
└── SKILL.md    ← AI に読ませるスキル本体(3 段階フローの手順 / frontmatter ルール / サブエージェント指示例)

リポジトリ直下:
├── temp/web/<article_topic>/  ← tav-cli が --topic ごとに蓄積する調査結果(役割別サブフォルダ構成は tav-cli スキル参照)
└── zenn/
...
> 調査フォルダの中身(役割別サブフォルダ・索引・本文ファイルの読み方)は `tav-cli` スキルが正本です。本スキルは「同じ `--topic` に蓄積し、その結果を読む」ところだけを規定します。
...
実際の調査手順、コマンド選択基準、サブエージェント指示テンプレ、plan / publish の同期ルールは [SKILL.md](SKILL.md) を参照してください。`tav` コマンド側の使い方・出力レイアウトは [claude-plugins/my-tools/skills/tav-cli/README.md](../../my-tools/skills/tav-cli/README.md) と [claude-plugins/my-tools/skills/tav-cli/SKILL.md](../../my-tools/skills/tav-cli/SKILL.md) が正本です。
