...
`tools/tav-cli/README.md` のセットアップ手順を案内する。
...
- Python スクリプトを直接叩く運用(`python .../search_topic.py ...`)はこのスキルでは行わない。常にインストール済みの `tav` コマンド経由で呼ぶこと。未インストールの場合は対処せず、`tools/tav-cli/README.md` のセットアップ手順をユーザーに案内する。
...
`char_count` は書き出した `.md` の文字数(`len()`)。`pages/index.json` を Read した時点でどの
ページを直接 Read し、どれを `aim-ask` にまとめて回すか(`tav-digest` スキル参照)を、
個別に開かず判断できる。
...
- `--topic` 指定時の読み方は**役割で違う**:
  ...
  - **content**(`pages/`): **まず `pages/index.json` を読み**、各エントリの `file`(`NNNN-<title>.md`)を順に開く。`.md` は `# <title>` + 本文(JSON ではなくそのまま読める)。`index.json` が唯一の url↔file 対応表。各エントリの `char_count` を見て、件数が多い/個々の文字数が大きい場合は1件ずつ Read せず `aim-ask` にまとめて回す(`tav-digest` スキル参照)。
...
- 全実行のフル詳細(リクエスト/レスポンス、投影前の生フィールド込み)は `tools/tav-cli/logs/<script>-log.json` に別途残る。`.env` の `TAVILY_WRITE_LOG`(未設定=`true`、`false`/`0`/`no`/`off`/空で抑止)で監査ログをトグルできる。
