...
# ]
# ///
"""Check that titles/URLs in the curated CLI excerpt still match the source llms.txt.

output/cli-excerpt.md is a hand-curated subset of the llms.txt pointed to by
config.yml's `source` key (see README.md in this directory for when/how to run
this script). That source llms.txt belongs to the ona-docs skill and is
re-downloaded periodically, so entries can drift: a page may get renamed/removed
...
It also lists source entries that look CLI-relevant (by keyword) but are not
...
bare `python`'s import path -- run it via `uv run check_cli_excerpt.py`, which
resolves the dependency declared below automatically.

Usage: uv run check_cli_excerpt.py [--excerpt PATH] [--source PATH]
...
DEFAULT_EXCERPT = SKILL_DIR / "output" / "cli-excerpt.md"
...
CANDIDATE_KEYWORDS = ("cli", "terminal", "command-line", "command line", "ona login")
...
def main() -> int:
    ...
    if candidates:
        print(
            f"POSSIBLE ADDITIONS ({len(candidates)}, informational only) -- source entries with CLI-ish keywords not in the excerpt; review manually, this is a keyword heuristic, not a re-curation:"
        )
        for e in candidates:
