...
# ]
# ///
"""Generate an initial draft of output/cli-excerpt.md from the source llms.txt
(path defined in config.yml) by asking an AI model (via the `aim` CLI) which entries
are about the Ona CLI (`ona` command).

check_cli_excerpt.py assumes output/cli-excerpt.md already exists -- it was
originally produced by hand-reviewing candidates found via keyword grep and this
script's draft output (see README.md). This script automates producing that
initial draft, e.g. for regenerating the excerpt from scratch after a major
reorganization of the upstream docs where most of the existing curation no
longer applies.

To avoid the title/description drift problem check_cli_excerpt.py exists to catch,
...
source `## Section` headings in source order (ona-docs' llms.txt currently has
only a couple of top-level sections, so in practice this draft ends up mostly flat).
...
miscuration before it replaces the real output/cli-excerpt.md -- this script
refuses to overwrite an existing file unless --force is given.

Usage: uv run generate_cli_excerpt.py [--model {mini-m3,gpt-120b,glm-5.2,gpt-luna}]
...
import check_cli_excerpt as base
...
SUSPICIOUS_RATIO = 0.4  # a general-docs chunk being mostly CLI-relevant is implausible
...
def build_excerpt(
    ...
    lines = [
        ...
        f"generated_by: generate_cli_excerpt.py (aim --model {model})",
        "note: >",
        "  DRAFT generated automatically by asking an AI model which entries in",
        "  ona-docs/output/docs/llms.txt relate to the Ona CLI (`ona` command).",
        "  This has NOT been reviewed by a human yet -- treat it as a starting",
        "  point, not a replacement for the curated output/cli-excerpt.md. Review",
        "  every entry, then run check_cli_excerpt.py to confirm titles/URLs match,",
...
def main() -> int:
    ...
    if not args.source.is_file():
        ...
        print(
            "Run the ona-docs skill's download step first (webref_cli.py download).",
            file=sys.stderr,
        )
        return 2
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        if _looks_suspicious(chunk_included, chunk):
            print(
                ...
                "implausibly high (a general docs chunk being mostly CLI-relevant is unlikely -- "
    ...
    print(
        ...
        "mis-curated entries, then run check_cli_excerpt.py to confirm titles/URLs "
        "match before treating it as the real output/cli-excerpt.md."
    )
    return 0
