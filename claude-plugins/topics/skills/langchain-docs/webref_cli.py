...
only bundles a download script (see SKILL.md's "no llms-full.txt-style full
text dump" note for why the excerpt/section-extraction templates were not
copied here), so SUBCOMMANDS below has a single row.
...
SUBCOMMANDS: dict[str, tuple[str, str]] = {
    "download": (
        "download_langchain_reference",
        "Fetch the LangChain llms.txt / llms-full.txt snapshot (freshness-checked, --force to refetch).",
    ),
}
...
def _root() -> None:
    ...
    working even while this skill only bundles one subcommand.
    """
