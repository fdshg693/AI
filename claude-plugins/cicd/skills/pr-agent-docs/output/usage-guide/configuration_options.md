...
Parameters from a local `.pr_agent.toml` file, in a specific repo, will override the global configuration parameters (the global file is merged _beneath_ the repo-local one).
...
This means an external URL acts as an organization-wide _default_ that any team can still override with their own `pr-agent-settings` or repo-local `.pr_agent.toml`.
