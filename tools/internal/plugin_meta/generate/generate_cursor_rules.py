...
context. The repository-root AGENTS.md is written as
.cursor/rules/agents-root.mdc and uses the catch-all pattern **.

Project rules must use the .mdc extension (a plain .md file under
.cursor/rules is ignored by Cursor because it has no rule frontmatter).
"""
...
def path_pattern(agent_path: Path, repo_root: Path) -> str:
    """Build the Cursor glob for files governed by an AGENTS.md."""
...
def output_path(agent_path: Path, repo_root: Path, output_dir: Path) -> Path:
    ...
    return output_dir / f"{relative_name}.mdc"


def render_rule(agent_path: Path, repo_root: Path) -> str:
    """Render one Cursor rule while preserving the AGENTS.md body."""

    globs = json.dumps(path_pattern(agent_path, repo_root), ensure_ascii=False)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Cursor project rules from repository AGENTS.md files."
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Generated rule directory (default: ai-tools.yaml's cursor.rules_from_agents_md.output_dir)",
    )
...
def main() -> None:
    ...
    default_output_dir = repo_root / load_tool("cursor")["rules_from_agents_md"]["output_dir"]
    output_dir = args.output_dir or default_output_dir
    output_dir = output_dir.resolve()
    count = generate(repo_root, output_dir)
    print(f"Generated {count} Cursor rules in {output_dir}")
