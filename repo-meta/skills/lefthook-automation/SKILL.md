---
# 対象: リポジトリルートのlefthook.yml（pre-commitフック一式）
name: lefthook-automation
...
meta:
  requires_repo_tools: lefthook.yml, tools/internal/justfile
  requires_env: none
  dependencies: none
  requires_install: lefthook, just
  requires_hooks: lefthook
  requires_skills: ai-tools-config, skill-md-commits
  status: stable
...
- `ai-tools.yaml`由来の各種生成ファイルの再生成（`marketplace.json`×2、`skill-catalog.json`、各`CATALOG.md`、Clineルール、Copilot instructions、READMEのAIツール節。詳細は[ai-tools-config](../ai-tools-config/SKILL.md)）
- `SKILL.md`の`meta.version`/`meta.description`デフォルト値のバックフィル
- `meta.version`が未バンプのままステージされた`SKILL.md`があればコミットをブロック（[skill-meta-fields](../skill-meta-fields/SKILL.md) / [skill-md-commits](../skill-md-commits/SKILL.md)参照）
...
`skill_meta_field_fill.py`（[skill-meta-fields](../skill-meta-fields/SKILL.md)、内部でaim-askを呼びOpenRouter APIを叩く）は、意図的にlefthookへ組み込まれていない。コミットのたびに課金を伴うAI呼び出しが走らないよう、明示呼び出し専用（該当スキルの`disable-model-invocation: true`）に留める。「自動化を最大限に活用する」の対象は、決定論的で無料/低コストな整形・再生成に限る。
...
既存の`ai-tools.yaml`由来ジョブと同じ形で、SSOTファイルと生成トリガーになりうるファイルの両方を`glob`に含めたlefthookジョブを追加する。この設計思想自体は[repo-ssot-pattern](../repo-ssot-pattern/SKILL.md)参照。

## 関連

- [ai-tools-config](../ai-tools-config/SKILL.md) — 生成される個々のファイルと生成スクリプトの詳細
- [skill-md-commits](../skill-md-commits/SKILL.md) — `SKILL.md`をコミットする際にlefthookが実行する内容と、詰まったときの直し方
- [repo-ssot-pattern](../repo-ssot-pattern/SKILL.md) — SSOT→生成スクリプト→lefthookという一連の設計パターン
