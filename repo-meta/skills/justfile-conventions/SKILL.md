---
# 対象: リポジトリ内の全justfile（ルート/tools/internal/tools/install/tools/my-agents/.claude/scripts/skills-site等）
...
meta:
  requires_repo_tools: justfile, tools/internal/justfile, tools/install/justfile, tools/my-agents/justfile, .claude/scripts/justfile, mise.toml
  requires_env: none
  dependencies: none
  requires_install: just, mise
...
このリポジトリには複数の`justfile`が共存する。ルート[`justfile`](../../../justfile)（`py-format`/`py-lint`/`skills-site-*`/Azureインフラ等リポジトリ横断のもの）、[`tools/internal/justfile`](../../../tools/internal/justfile)（このリポジトリ自身のメタ生成、[ai-tools-config](../ai-tools-config/SKILL.md)参照）、そして`tools/install/justfile`・`tools/my-agents/justfile`・`.claude/scripts/justfile`・`skills-site/justfile`のような、そのディレクトリの関心事に閉じた小さなものに分かれる。
...
- [ai-tools-config](../ai-tools-config/SKILL.md) — `tools/internal/justfile`の各レシピが何を再生成するか
- [lefthook-automation](../lefthook-automation/SKILL.md) — justfileレシピをコミット時に自動実行させる仕組み
- [tools-directory-layout](../tools-directory-layout/SKILL.md) — ツールのインストールコマンドをjustfileレシピとして揃える規約
