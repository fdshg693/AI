---
# 使い方自体はclaude-plugins/my-tools/skills/aim-cli等参照。ここでは「いつaimファミリーに頼るか」の方針を扱う
name: aim-automation
...
meta:
  requires_repo_tools: tools/aim, tools/aim-use/aim-ask, tools/aim-use/aim-summarize
  requires_env: OPENROUTER_API_KEY
  dependencies: none
  requires_install: uv tool install --editable tools/aim, uv tool install --editable tools/aim-use/aim-ask, uv tool install --editable tools/aim-use/aim-summarize
  requires_hooks: none
  requires_skills: aim-cli, aim-ask, aim-summarize
  status: stable
...
`skill_meta_field_fill.py`（[skill-meta-fields](../skill-meta-fields/SKILL.md)）は、各`SKILL.md`をエージェントに1件ずつ読ませて判断させる代わりに、`aim-ask`でスキルディレクトリ単位に投げて`meta:`の値を埋める。
...
- [skill-meta-fields](../skill-meta-fields/SKILL.md) — このリポジトリ自身がaim-askを使っている例
