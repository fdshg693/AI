...
- **非対話（`-p`/`--print`）の単発実行のみ**を対象にした。対話的なREPL利用（`agent`をそのまま起動して会話する使い方）は別物として意図的に除外している。理由: このスキルはClaude Codeが「他のCLIエージェントに一撃タスクを委譲する」ためのものであり、対話セッションの管理（履歴・`/model`切り替え等）はスコープが異なるため。
