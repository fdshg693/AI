...
- **Hex Canvas** 窶・Build data visualizations. At Cursor, we use the Hex Canvas to explore and share analytics.
- **Atlassian Canvas** 窶・See a realtime view of your issues, projects, and documents from Jira and Confluence.
...
Auto Refresh updates plugins that are already part of the marketplace. Adding a brand-new plugin from the repository isn't automatic 窶・re-import the repository URL to pick up newly added plugins.
...
笏・  笏披楳笏 plugin.json
笏懌楳笏 rules/
笏・  笏披楳笏 coding-standards.mdc
笏懌楳笏 skills/
笏・  笏披楳笏 code-reviewer/
笏・      笏披楳笏 SKILL.md
笏披楳笏 mcp.json
```
