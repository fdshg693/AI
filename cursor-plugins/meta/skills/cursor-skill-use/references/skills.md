...
笏披楳笏 skills/
    ...
    笏・  笏懌楳笏 land-it/
    笏・  笏・  笏披楳笏 SKILL.md
    笏・  笏披楳笏 careful-merge-conflicts/
    笏・      笏披楳笏 SKILL.md
    笏懌楳笏 debugging/
    笏・  笏披楳笏 using-datadog-mcp/
    笏・      笏披楳笏 SKILL.md
    笏披楳笏 workflow/
        笏披楳笏 tdd/
            笏披楳笏 SKILL.md
...
The agent reads these instructions and executes the referenced scripts when the skill is invoked. Scripts can be written in any language窶韮ash, Python, JavaScript, or any other executable format supported by the agent implementation.
...
Keep your main `SKILL.md` focused and move detailed reference material to separate files. This keeps context usage efficient since agents load resources progressively窶俳nly when needed.
...
- **Dynamic rules**: Rules that use the "Apply Intelligently" configuration窶排ules with `alwaysApply: false` (or undefined) and no `globs` patterns defined. These are converted to standard skills.
