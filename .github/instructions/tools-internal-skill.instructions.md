---
name: "tools/internal/skill instructions"
description: "Instructions for files in tools/internal/skill/"
applyTo: "tools/internal/skill/**"
---
...
- `check/` — lefthook等から呼ばれる検査専用スクリプト（`check_skill_version_bump.py`, `check_skill_repo_tools.py`）。ファイルを書き換えない
- `util/` — 上記が共有するヘルパー（frontmatterの読み書き、`meta_field.yaml`ローダ、`repo-tools.yaml`ローダ（`repo_tools_registry.py`）等）。`set/`・`check/`以外から新規に依存を追加しない
