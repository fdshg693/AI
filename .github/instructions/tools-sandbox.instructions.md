---
name: "tools/sandbox instructions"
description: "Instructions for files in tools/sandbox/"
applyTo: "tools/sandbox/**"
---
...
- `docker/.env.example` — コンテナに注入する環境変数の一覧
...
- `orchestrator/run_agent.py` — ホスト/コンテナ両モードを持つ1ファイル。ホスト側は`docker run`でISSUE専用コンテナを起動し、コンテナ側（`--container-mode`）はclone〜Claude Agent SDK `query()`〜push を行う
...
- `orchestrator/state.py` — ISSUEごとの試行記録（1 ISSUE = 1回まで）をSQLiteで管理する`AttemptStore`。手動リセット用CLI（`python state.py --reset <issue_number>`）も兼ねる
